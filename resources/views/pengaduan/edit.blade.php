...
<div class="bg-white py-6 px-9 mb-5 rounded-lg">
    <form action="/pengaduan/{{ $pengaduan->id }}" method="POST">
        ...
        <label class="block mb-6">
            ...
            <input type="text" name="masyarakat_id" class="mt-1 px-3 py-2 bg-white border shadow-sm border-gray placeholder-secondary focus:outline-none focus:border-gray focus:ring-gray block w-full rounded-md sm:text-sm focus:ring-1 text-secondary " placeholder="you@example.com" value="{{ $pengaduan->masyarakat->username }}" />
        </label>
        
        <label for="" class="after:content-['*'] after:ml-0.5 after:text-danger block mb-2 text-sm font-medium text-dark">Isi Laporan</label>
        <textarea id="isi_laporan" name="isi_laporan" rows="4" class="mb-6 block px-3 py-2 w-full focus:outline-none text-sm text-secondary bg-white rounded-lg border border-gray shadow-sm focus:border-gray focus:ring-gray" placeholder="Ketik laporan...">{{ $pengaduan->isi_laporan }}</textarea>
